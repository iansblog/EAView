...
window.renderDeliveryProgramsGantt = function() {
  fetch('/data/asis.json')
    .then(r => r.json())
    .then(data => {
      // Collect all systems with deliveryProgramId
      const systems = Object.entries(data.systems).map(([id, sys]) => ({ id, ...sys }));
      // Group by deliveryProgramId
      const programs = {};
      systems.forEach(sys => {
        if (sys.deliveryProgramId) {
          if (!programs[sys.deliveryProgramId]) programs[sys.deliveryProgramId] = [];
          programs[sys.deliveryProgramId].push(sys);
        }
      });
      // Get program names
      const programNames = Object.fromEntries(Object.entries(data.deliveryPrograms||{}).map(([id, p]) => [id, p.name]));
      // Flatten for Gantt: one bar per system
      const bars = [];
      Object.entries(programs).forEach(([progId, sysArr]) => {
        sysArr.forEach(sys => {
          (sys.contracts||[]).forEach(contract => {
            bars.push({
              program: programNames[progId] || progId,
              system: sys.name,
              status: sys.status,
              criticality: sys.criticality,
              owner: sys.owner,
      ...
      // Filtering controls
      const crits = Array.from(new Set(bars.map(b => b.criticality).filter(Boolean)));
      const owners = Array.from(new Set(bars.map(b => b.owner).filter(Boolean)));
      d3.select('#filter-criticality').selectAll('option').data(['All',...crits]).join('option').attr('value',d=>d).text(d=>d);
      d3.select('#filter-owner').selectAll('option').data(['All',...owners]).join('option').attr('value',d=>d).text(d=>d);
      function updateGantt() {
        let filtered = bars;
        const crit = d3.select('#filter-criticality').property('value');
        const owner = d3.select('#filter-owner').property('value');
        if (crit && crit !== 'All') filtered = filtered.filter(b => b.criticality === crit);
        if (owner && owner !== 'All') filtered = filtered.filter(b => b.owner === owner);
        // D3 Gantt
        const container = d3.select('#programs-gantt');
        container.selectAll('*').remove();
        const width = container.node().clientWidth || 900;
        const height = Math.max(filtered.length * 32 + 60, 200);
        const margin = { top: 40, right: 40, bottom: 40, left: 220 };
        const svg = container.append('svg').attr('width', width).attr('height', height);
        // X scale
        const minDate = d3.min(filtered, d => d.start);
        const maxDate = d3.max(filtered, d => d.end);
        const x = d3.scaleTime().domain([minDate, maxDate]).range([margin.left, width - margin.right]);
        // Y scale
        const y = d3.scaleBand().domain(filtered.map(d => d.system)).range([margin.top, height - margin.bottom]).padding(0.2);
        // X axis
        svg.append('g')
          .attr('transform', `translate(0,${margin.top - 10})`)
          .call(d3.axisTop(x).ticks(width/120).tickFormat(d3.timeFormat('%b %Y')));
      ...
      d3.select('#filter-criticality').on('change', updateGantt);
      d3.select('#filter-owner').on('change', updateGantt);
      updateGantt();
    });
};
